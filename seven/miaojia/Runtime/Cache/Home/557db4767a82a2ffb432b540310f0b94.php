...
<div class="car clearfix frm_sty">
	...
	<!-- 侧边栏导航 结束 -->
		<ul class="fl">
			...
			<li class="no"><a href="<?php echo u('User/addressList');?>">收货地址</a></li>
	...
	<!-- 收货地址 开始 -->
		<div class=" order">
            ...
            <?php if(is_array($addressList)): $i = 0; $__LIST__ = $addressList;if( count($__LIST__)==0 ) : echo "您还没有添加收货地址" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><ul class="clearfix">
                    <li>
                        <p class="p0">收货地址:<?php echo ($vo["address"]); ?>,收货人:<?php echo ($vo["name"]); ?>,联系电话:<?php echo ($vo["tel"]); ?></p>
                        <p><a href="<?php echo u('delAddress',array('id'=>$vo['id']));?>">删除</a> </p>
                    </li>
                </ul><?php endforeach; endif; else: echo "您还没有添加收货地址" ;endif; ?>
            <?php echo ($page); ?>
		</div>
	<!-- 收货地址 结束 -->
	<!-- 我的关注 开始 -->
		<div class="attention clearfix">
			...
			<ul>
				<li class="li01">
					<a href="#">
						<p class="p1"></p>
						<p class="p2">苗族农家自制萝卜<br/>干 细干萝卜条</p>
					</a>
				</li>
			</ul>
		</div>
