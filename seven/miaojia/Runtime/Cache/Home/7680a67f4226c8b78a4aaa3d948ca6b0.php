...
<head>
	...
	<title>我的收藏-<?php echo C('WEB_NAME');?></title>
...
<div class="car clearfix frm_sty">
	...
	<!-- 侧边栏导航 结束 -->
		<ul class="fl">
			...
			<li class="no"><a href="<?php echo u('User/goodsScList');?>">商品收藏</a></li>
	...
	<!-- 收货地址 开始 -->
		<div class=" order">
            ...
            <?php if(is_array($goodsScList)): $i = 0; $__LIST__ = $goodsScList;if( count($__LIST__)==0 ) : echo "您还没有收藏的商品" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><ul class="clearfix">
                    <li>
                        <p class="p0"><img src="/upload/n3/<?php echo (array_shift(explode(',',(isset($vo["image"]) && ($vo["image"] !== ""))?($vo["image"]):'default.jpg'))); ?>"> </p>
                        <p><a href="<?php echo u('Goods/detail',array('id'=>$vo['id']));?>" target="_blank"><?php echo (mb_substr($vo["goodsname"],'0','20','utf-8')); ?></a></p>
                        <p><?php echo ($vo["siteprice"]); ?>元</p>
                        <p><a href="javascript:void(0)" rel="<?php echo ($vo["id"]); ?>" class="delSc">删除</a> </p>
                    </li>
                </ul><?php endforeach; endif; else: echo "您还没有收藏的商品" ;endif; ?>
            <?php echo ($page); ?>
		</div>
	<!-- 收货地址 结束 -->
	</div>
