...
protected $_map=array(
    ...
    protected $_validate=array(
       //array('验证字段','验证规则','错误提示',['验证条件','附加规则','验证时间']);
        ...
        array('email','/^[\w-]+@([\w-]+\.)+[a-zA-Z]+$/','邮箱格式错误',0,'regex'),
        array('tel','/^1[34578]\d{9}$/','手机号不正确',0,'regex') // 在新增的时候验证手机号字段是否为空
...
//个人信息
   public function memberinfo($username){
        $User=M('User');
    ...
    public function changepwd(){
        ...
        if($this->where(array('username'=>session('username'),'pwd'=>md5($data['pwd'])))->find()){

            $this->where(array('username'=>session('username')))->field('pwd')->save($data);
