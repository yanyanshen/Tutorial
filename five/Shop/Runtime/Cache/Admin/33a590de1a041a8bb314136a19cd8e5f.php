    ...
    <script type="text/javascript">
        ...
        function del(id){
            layer.confirm("你确定要删除我吗?",{icon:4,btn:['确定','取消']},function(){
                $.get("<?php echo U('AdminNav/delete');?>",{id:id},function(res){
                    if(res.status=="ok"){
                        layer.msg(res.msg,{icon:6,time:2000},function(){
                            window.location.href="<?php echo U('AdminNav/showlist');?>";
                        });
                    }else{
                        layer.alert(res.msg);
                    }
    ...
    <div id="usual1" class="usual">
        <div id="tab2" class="tabson">
            <table class="tablelist">
                ...
                <tbody>
                <?php if(is_array($navList)): $i = 0; $__LIST__ = $navList;if( count($__LIST__)==0 ) : echo "暂时没有数据供你查看" ;else: foreach($__LIST__ as $key=>$value): $mod = ($i % 2 );++$i;?><tr>
                        ...
                        <td>
                            <a href="<?php echo U('AdminNav/showlist',array('id'=>$value['id']));?>">
                                <?php echo str_repeat('&nbsp;',0*($value['level']-1));?>|--<?php echo ($value["navname"]); ?>
                            </a>
                        </td>
                        <td><?php echo ($value["navurl"]); ?></td>
                        <td>
                            <a href="<?php echo U('addlist',array('pid'=>$value['id'],'pname'=>$value['navname']));?>" class="tablelink">添加子菜单</a> |
                            <a href="<?php echo U('AdminNav/showlist',array('id'=>$value['id']));?>" class="tablelink">查看子菜单</a> |
                    ...
                    </tr><?php endforeach; endif; else: echo "暂时没有数据供你查看" ;endif; ?>
                </tbody>
            </table>
        </div>
    </div>
