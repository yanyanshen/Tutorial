    ...
    <script>
        function enabled(gid){
            layer.confirm("确定要更改吗?",{
                ...
                $.get("<?php echo U('Goods/enabled');?>","id="+gid,function(res){
                    if(res.status=="ok"){
                        layer.msg(res.msg,{icon:1,time:1000},function(){
                            window.location.href="<?php echo U('Goods/showlist');?>";
                        });
        ...
        function disabled(gid){
            layer.confirm("确定要更改吗?",{
                ...
                $.get("<?php echo U('Goods/disabled');?>","id="+gid,function(res){
                    if(res.status=="ok"){
                        layer.msg(res.msg,{icon:1,time:1000},function(){
                            window.location.href="<?php echo U('Goods/showlist');?>";
                        });
...
//                        layer.msg(res.msg,{icon:2,time:1000});
        ...
        function addrecycle(gid){
            layer.confirm("确定要加入到回收站吗?",{
                ...
                $.get("<?php echo U('Goods/addrecycle');?>","id="+gid,function(res){
                    if(res.status=='ok'){
                        layer.msg(res.msg,{icon:1,time:1000},function(){
                            window.location.href="<?php echo U('Goods/showlist');?>";
                        })
...
<body>
    ...
    <form action="<?php echo U('Admin/Goods/showlist');?>" method="get">
    <ul class="seachform">
    ...
    <table class="tablelist">
        ...
        <tbody>
        <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "<?php echo ($empty); ?>" ;else: foreach($__LIST__ as $key=>$value): $mod = ($k % 2 );++$k;?><tr>
        <td><input name="" type="checkbox" value="" /></td>
        <td><?php echo ($k+$firstRow); ?></td>
        <td><img src="/Public/Admin/Uploads/goods/<?php echo ($value["pic"]); ?>" width="50px" height="50px" alt=""/></td>
        ...
        <td><?php echo ($value["salenum"]); ?></td>
        <td><?php echo date('Y-m-d H:i:s',$value['addtime']);?></td>
        <?php if($value['display'] == 1): ?><td>上架</td>
            <?php else: ?>
            <td>下架</td><?php endif; ?>
        <td><a href="<?php echo U('Admin/Goods/edit',array('id'=>$value['id']));?>" class="tablelink">编辑</a>
            ...
            <a href="javascript:del(<?php echo ($value['id']); ?>)" class="tablelink">彻底删除</a>
            <a href="javascript:addrecycle(<?php echo ($value['id']); ?>)" class="tablelink">加入回收站</a>
        </td>
        </tr><?php endforeach; endif; else: echo "<?php echo ($empty); ?>" ;endif; ?>
        </tbody>
    </table>
        <div class="pagin">
