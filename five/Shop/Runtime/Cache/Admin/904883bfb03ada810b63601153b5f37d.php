...
<body>
    ...
    <div id="usual1" class="usual"> 
  	<div id="tab1" class="tabson">
        <form action="<?php echo U('Admin/Goods/addlist');?>" method="post" enctype="multipart/form-data" id="form1">
            <ul class="forminfo">
                ...
                <li><label>选择分类<b>*</b></label>
                    <div class="vocation">
                        <select class="select1" name="cid"><option value="0">顶级分类</option>
                            <?php if(is_array($category)): $i = 0; $__LIST__ = $category;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><option value="<?php echo ($val['id']); ?>"><?php echo ($val['catename']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                            </select></div></li>
                <li><label>选择商标<b>*</b></label>
                    <div class="vocation">
                        <select class="select1" name="bid"><option value="0">品牌选择</option>
                            <?php if(is_array($brand)): $i = 0; $__LIST__ = $brand;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><option value="<?php echo ($val['id']); ?>"><?php echo ($val['bname']); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                        </select></div></li>
        ...
        $(this).ajaxSubmit(function(res) {
            if(res.status=='gnull'){
                layer.alert("商品名称不能为空!",{icon:2},function(){
                    window.location.href="<?php echo U('Goods/addlist');?>";
                })
