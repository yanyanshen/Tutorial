                    ...
                    <ul class="tab_conbox">
                        <li id="tab1" class="tab_con">
                            <table class="tablelist">
                                ...
                                <tbody>
                                <?php if(is_array($myAuctionRecord)): $k1 = 0; $__LIST__ = $myAuctionRecord;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val1): $mod = ($k1 % 2 );++$k1;?><tr>
                                        <td><?php echo ($k1); ?></td>
                                        <td>
                                            <img src="/Public/Admin/Uploads/goods/<?php echo ($val1['pic']); ?>" style="width: 50px;height: 50px;">
                                        </td>
                                        <td><?php echo (mb_substr($val1['goodsname'],0,15,"utf-8")); ?></td>
                                        <td><?php echo ($val1['auctionprice']); ?></td>
                                        <td><?php echo ($val1['buynum']); ?></td>
                        ...
                        <li id="tab2" class="tab_con">
                            <table class="tablelist">
                                ...
                                <tbody>
                                <?php if(is_array($getAuctionGoods)): $k2 = 0; $__LIST__ = $getAuctionGoods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val2): $mod = ($k2 % 2 );++$k2;?><form action="" method="post" id="auctionForm<?php echo ($k2); ?>">
                                        <input name="total_price" value="<?php echo ($val2['pay']); ?>" type="hidden">
                                        <input name="checkitems[]" value="<?php echo ($val2['gid']); ?>" type="hidden">
                                        <input name="ag_id" value="<?php echo ($val2['ag_id']); ?>" type="hidden">
                                        <input name="pay" value="<?php echo ($val2['pay']); ?>" type="hidden">
                                        <input name="buynum<?php echo ($val2['gid']); ?>" value="<?php echo ($val2['buynum']); ?>" type="hidden">
                                    <tr>
                                        <td><?php echo ($k2); ?></td>
                                        <td><img src="/Public/Admin/Uploads/goods/<?php echo ($val2['pic']); ?>" style="width: 50px;height: 50px;"></td>
                                        <td><?php echo ($val2['goodsname']); ?></td>
                                        <td><?php echo ($val2['price']); ?></td>
                                        <td><?php echo ($val2['buynum']); ?></td>
                                        <td><?php echo ($val2['deposit']); ?></td>
                                        <td><?php echo ($val2['pay']); ?></td>
                                        <td>
                                            <?php if($val2['isshow'] == 1): ?><a href="javascript:pay(<?php echo ($k2); ?>)">去付款</a>
                                                <?php else: ?>
                                                <a href="<?php echo U('Personal/order');?>">查看订单</a><?php endif; ?>
                                        </td>
                                    </tr>
...
<script>
    function comment(gid,oid){
        layer.open({
            ...
            title:'评价',
            skin:'demo-class',
            content:"<?php echo U('Personal/commentlist');?>?gid="+gid+"&&oid="+oid
        });
    }
    function del(mid){
        layer.confirm('是否删除',{icon:3,title:'删除'},function(){
            $.get("<?php echo U('Personal/del');?>","mid="+mid,function(res){
                if(res.status=="ok"){
                    layer.msg(res.msg,{icon:1,time:1000},function(){
                        window.location.href="<?php echo U('Personal/comment');?>";
                    })
    ...
    //去付款
    function pay(k){
        $.post("<?php echo U('Order/createOrder');?>",$("#auctionForm"+k).serialize(),function(res){
            if(res.status=="ok"){
                layer.msg(res.msg,{icon:1,time:2000},function(){
                    window.location.href="<?php echo U('Order/showlist');?>?oid="+res.oid+"&pay="+res.pay;
                })
            }else{
