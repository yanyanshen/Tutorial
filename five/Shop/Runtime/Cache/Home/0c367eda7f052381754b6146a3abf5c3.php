                        ...
                        <div class="adderss_style">
                            ...
                            <div class="adderss_list">
                                ...
                                <div class="Address_List clearfix">
                                    <!--地址类表-->
                                    <?php if(is_array($addressInfo)): $i = 0; $__LIST__ = $addressInfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$addVal): $mod = ($i % 2 );++$i; if($addVal['isdefault'] == 1): ?><ul class="Address_info">
                                            <div class="address_title">
                                                <a href="javascript:updateAddress(<?php echo ($addVal['id']); ?>)" class="modify iconfont icon-fankui btn btn-primary" title="修改信息" data-toggle="modal" id="purebox"></a>
                                                <span style="color:#FF7200">默认地址</span>
                                                <a href="javascript:del(<?php echo ($addVal['id']); ?>)" class="delete "><i class="iconfont icon-close2"></i></a></div>
                                            <li><?php echo ($addVal["name"]); ?></li>
                                            <li><?php echo ($addVal["address"]); ?></li>
                                            <li><?php echo ($addVal["mobile"]); ?></li>
                                            <li><?php echo ($addVal["postcode"]); ?></li>
                                        </ul>
                                    <?php else: ?>
                                        <ul class="Address_info">
                                            <div class="address_title">
                                                <a href="javascript:setDefaultAdd(<?php echo ($addVal['id']); ?>)" class="modify iconfont icon-fankui btn btn-primary" title="设置为默认地址" data-toggle="modal" id="fujia"></a>
                                                附加地址
                                                <a href="javascript:del(<?php echo ($addVal['id']); ?>)" class="delete "><i class="iconfont icon-close2"></i></a></div>
                                            <li><?php echo ($addVal["name"]); ?></li>
                                            <li><?php echo ($addVal["address"]); ?></li>
                            ...
                            <form action="<?php echo U('Address/editAddress');?>" method="post" id="addressForm">
...
<script type="text/javascript">
    ...
    $(function(){
        $(".Add_btn").click(function(){
            $.post("<?php echo U('Home/Address/editAddress');?>",$("#addressForm").serialize(),function(res){
                if(res.status=="ok"){
                    layer.msg(res.msg,{icon:1,time:1000},function(){
                        window.location.href="<?php echo U('Personal/address');?>";
                    })
                }else{
  ...
  function del(aid){
      $.get("<?php echo U('Address/del');?>",{id:aid},function(res){
          if(res.status=="ok"){
              layer.msg(res.msg,{icon:1,time:1000},function(){
    ...
    function setDefaultAdd(aid){
        $.get("<?php echo U('Address/setDefault');?>",{id:aid},function(res){
            if(res.status=="ok"){
                layer.msg(res.msg,{icon:1,time:1000},function(){
                    window.location.href="<?php echo U('Personal/address');?>";
                })
    ...
    function updateAddress(aid){
        layer.open({
            ...
            shadeClose: true,
            shade: 0.8,
            content:"<?php echo U('Address/updateAddress');?>?id="+aid
        })
    }
