        ...
        <div class="right_style">
            <div class="info_content">
                <div class="collect_style r_user_style">
                    ...
                    <div class="collect">
                        <ul class="Quantity"><li>已藏量：<?php echo ($collectnum); ?>条</li><li></li></ul>
                        <div class="collect_list">
                            <ul>
                                <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$val): $mod = ($k % 2 );++$k;?><li class="collect_p">
                                    <a href="javascript:collectdel(<?php echo ($val['id']); ?>);"><em class="iconfont icon-close2 delete"></em></a>
                                    <a href="<?php echo U('Detail/detail',array('gid'=>$val['gid']));?>" class="buy_btn">查看详情</a>
                                    <div class="collect_info">
                                        <div class="img_link"> <a href="#" class="center "><img src="/Public/Admin/Uploads/goods/thumb350/350_<?php echo ($val['pic']); ?>"></a></div>
                                        <dl class="xinxi">
                                            <dt><a href="#" class="name"><?php echo ($val["goodsname"]); ?></a></dt>
                                            <dd><span class="Price"><b>￥</b><?php echo ($val["price"]); ?></span><span class="collect_Amount"><i class="iconfont icon-shoucang"></i><?php echo ($val["salenum"]); ?></span></dd>
                                        </dl>
                                    </div>
                                </li><?php endforeach; endif; else: echo "$empty" ;endif; ?>
                            </ul>
                        </div>
                        <div id="footPage"><?php echo ($page); ?></div>
                    </div>
                             </div>
                         </div>
                     </div>
...
<script>
    function collectdel(cid){
        $.get("<?php echo U('Personal/collectdel');?>","cid="+cid,function(res){
            if(res.status==1){
                layer.msg(res.msg,{icon:6,time:500},function(){
                    window.location.href="<?php echo U('Personal/collection');?>";
                })
