...
<head lang="en">
    ...
    <style>
        *{margin:0;padding:0;list-style-type:none;}
        ...
        #star li{float:left;width:24px;cursor:pointer;text-indent:-9999px;background:url(/Public/Home/images/star.png) no-repeat;}
        #star strong{color:#f60;padding-left:10px;}
        #star li.on{background-position:0 -28px;}
        #star p{position:absolute;top:20px;width:159px;height:60px;display:none;background:url(/Public/Home/images/icon.gif) no-repeat;padding:7px 10px 0;}
    ...
    <link rel="stylesheet" type="text/css" href="/Public/Home/webuploader/0.1.5/webuploader.css" />
    <script type="text/javascript" src="/Public/Home/webuploader/0.1.5/webuploader.js"></script>
    ...
    <script type="text/javascript">
        ...
        var listUrl = '<?php echo U("commentlist");?>';
...
<body>
<form action="<?php echo U('Personal/commentlist');?>" method="post" id="form1">
    <div id="star">
        <span>请对商品打分</span>
        <ul>
...
<!--<script>
    $(function(){
        $("#btn").click(function(){
            var start = $('#start').html();
            $("#star").append("<input type='hidden' name='start' value='"+start.substr(8,1)+"'>");
            $.post("<?php echo U('Personal/commentlist');?>",$("#form1").serialize(),function(res){ //将form1中数据提交到后台
                if(res.status=="ok"){
                    layer.msg(res.msg,{icon:1,time:1000},function(){
                        parent.location.href="<?php echo U('Personal/comment');?>";
                    })
                }
                else if(res.status=="cnull"){
                    layer.msg(res.msg,{icon:1,time:1000},function(){
