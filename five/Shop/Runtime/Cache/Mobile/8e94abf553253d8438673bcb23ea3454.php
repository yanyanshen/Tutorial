...
<body>
	...
	<div class="login">
		<form action="<?php echo U('Personal/editPayPwd');?>" method="post" id="form1">
			
			<ul>
				<li>
					<img src="/Public/Mobile/images/login.png"/>
					<input name="paypwd" type="password" placeholder="输入原密码"/>
				</li>
				<li>
					<img src="/Public/Mobile/images/password.png"/>
					<input name="newpaypwd" type="password" placeholder="输入新密码"/>
				</li>
				<li>
					<img src="/Public/Mobile/images/password.png"/>
					<input name="renewpaypwd" type="password" placeholder="输入确认密码"/>
				</li>
			</ul>
...
<script>
    $("#sub").click(function(){
        $.post("<?php echo U('Personal/editPayPwd');?>",$("#form1").serialize(),function(res){
            if(res.status=="ok"){
                //信息框
                layer.open({
                    content:res.msg
