        ...
        $(function(){
            $(".collectbox").click(function(){
                ...
                $('input:checkbox:checked').each(function(i){
                    if(i==0){spCodesTemp =$(this).val();}
                    else{spCodesTemp +=(","+$(this).val());}
                });
                layer.open({
                    content: '您确定要删除吗？'
                    ,btn: ['删除', '不要']
                    ,yes: function(){
                        $.get("<?php echo U('Personal/collectdel');?>","cid="+spCodesTemp,function(res){
                            if(res.status==1){
                                layer.open({
                                    content: res.msg
                                    ,skin: 'msg'
...
<body>
	...
	<div id="box" class="contaniner fixed-conta">
		...
		<section class="list">
			<ul class="wall">
                <?php if(is_array($collect)): $k = 0; $__LIST__ = $collect;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$value): $mod = ($k % 2 );++$k;?><li class="pic">
...
<script>
    ...
    function page(id){
        var id=id?id:1;
        $.get("<?php echo U('Personal/mycollect');?>",{"p":id},function(res){
            $('#box').html(res);
        })
    }
</script>
