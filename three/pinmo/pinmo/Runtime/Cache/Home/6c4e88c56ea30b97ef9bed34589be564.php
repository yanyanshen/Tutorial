    ...
    <div class="shop_member_bd_right clearfix">
        ...
        <div class="shop_meber_bd_good_lists clearfix">
            ...
            <table>
                ...
                <form action="<?php echo U('Home/Shopcar/discuss');?>" method="post" id="form">
                    <?php if(is_array($ordergoods)): $k = 0; $__LIST__ = $ordergoods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$ordergoods): $mod = ($k % 2 );++$k;?><tbody>

                                <tbody style="border: 1px solid #AED2FF">
                                <tr>
                                    <td style="text-align: center;" class="dingdan_pic"><img width="80px" src="/three13(2)/pinmo/Uploads/<?php echo ($ordergoods["goodspic"]); ?>" /></td>
                                    <td style="text-align: center;" class="dingdan_title"><span>
                                        <input type="hidden" name="goodsid<?php echo ($k); ?>" value="<?php echo ($ordergoods["goodsid"]); ?>"/>
                                        <textarea style="height: 100px;width: 570px;scrolling:none;font-size: 25px" name="msg<?php echo ($k); ?>" placeholder="在这里输入您的评价吧！"/></textarea></span></td>
                                </tr>
                                </tbody>
...
<script>
    $('#ok').click(function(){
        alert($('#form').serialize());
            $.post("<?php echo U('Home/Shopcar/discuss');?>",$('#form').serialize(),function(res) {
                if (res.status == 'ok') {
                    layer.alert(res.msg, {
                        yes: function () {
