        ...
        <div id="tab2" class="tabson">
            <table class="tablelist" id="adlist">
                ...
                <?php if(is_array($goodsList)): $k = 0; $__LIST__ = $goodsList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($k % 2 );++$k;?><tr>
                        ...
                        <td><img src="/three17/pinmo/Uploads/<?php echo ($value["goodspic"]); ?>"  width="200px" height="100px"/></td>
                        <td><?php echo ($value["description"]); ?></td>
                        <td><?php echo ($value['issale']==1?'上架':'下架'); ?></td>
                        <td><a href="javascript:del(<?php echo ($value['id']); ?>);" class="tablelink">删除</a>
                            <a href="<?php echo u('Admin/Goods/edit',array('id'=>$value['id']));?>" class="tablelink">编辑</a>
                            <a href="javascript:issale(<?php echo ($value['id']); ?>);" class="tablelink"><?php echo ($value['issale']==1?'下架':'上架'); ?></a></td>
...
<script>
    function del(id) {
        layer.confirm('您是真的要删除吗？', {
            ...
            $.post("<?php echo U('Admin/Goods/del');?>", {'id': id}, function (res) {
                if (res.status == 'ok') {
                    layer.alert(res.msg, {
                        yes:function(){
    ...
    function issale(id){
        $.post("<?php echo U('Admin/Goods/issale');?>", {'id': id}, function (res) {
            if (res.status == 'ok') {
                layer.alert(res.msg, {
                    yes:function(){
                        location.href="<?php echo U('Admin/Goods/goodsList');?>";
