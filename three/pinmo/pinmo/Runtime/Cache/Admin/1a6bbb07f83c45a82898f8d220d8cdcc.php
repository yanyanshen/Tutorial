    ...
    <div id="usual1" class="usual">
        ...
        <div id="tab2" class="tabson">
            <table class="tablelist" id="adlist">
                ...
                <tbody>
                <?php if(is_array($userList)): $k = 0; $__LIST__ = $userList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$value): $mod = ($k % 2 );++$k;?><tr>
                        ...
                        <td><?php echo ($value['state']==1?'激活':'冻结'); ?></td>
                        <td><a href="javascript:del(<?php echo ($value['id']); ?>);" class="tablelink">删除</a>

                            <a href="javascript:state(<?php echo ($value['id']); ?>);" class="tablelink"><?php echo ($value['state']==1?'冻结':'激活'); ?></a></td>
...
<script>
    function del(id) {
        ...
        layer.confirm('您是真的要删除吗？', {
            ...
            $.post("<?php echo U('Admin/Users/del');?>", {'id': id}, function (res) {
                if (res.status == 'ok') {
                    layer.alert(res.msg, {
                        yes:function(){
                            location.href="<?php echo U('Admin/Users/userlist');?>";
                        }
                    });
                } else {
                    layer.alert(res.msg);
    ...
    function state(id){
        $.post("<?php echo U('Admin/Users/state');?>", {'id': id}, function (res) {
            if (res.status == 'ok') {
                layer.alert(res.msg, {
                    yes:function(){
                        location.href="<?php echo U('Admin/Users/userlist');?>";
