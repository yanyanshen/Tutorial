...
<form id="form1" action="<?php echo u('Home/Cart/submit_order');?>" method="post">
...
<!-- 送货清单 开始 -->
		...
		<div class="title clearfix">
			...
			<div class="goods01 fl" style="height: auto">
                <table width="1200px" style="text-align: center; vertical-align: middle" id="tb1">
                    ...
                    <?php if(is_array($goodsList)): $i = 0; $__LIST__ = $goodsList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$val): $mod = ($i % 2 );++$i;?><tr id="goods">
                            <td style="vertical-align: middle"><input type="hidden" name="gid" value="<?php echo ($val["gid"]); ?>"><img width="100px" height="100px" src="/Public/Upload/Goodspic/<?php echo ($val["pic"]); ?>"></td>
                            <td><a href="#"><?php echo mb_substr($val['goodsname'],0,15,'utf-8');?></a></td>
                            <td><p class="p3"><?php echo ($val["color"]); ?></p></td>
                            <td><p class="p1"><span><input type="hidden" value="<?php echo ($val["price"]); ?>" name="price">￥<?php echo ($val["price"]); ?></span></p></td>
                            <td><p class="p2"><span><input type="hidden" value="<?php echo ($buynum); ?>" name="buynum"><?php echo ($buynum); ?></span></p></td>
    ...
    <div class="privilege frm_sty clearfix">
        <div class="privilegeCont fr ">
            <p><span><?php echo ($buynum); ?></span>件 商品总金额：￥<?php echo ($sum); ?></p>
        </div>
    </div>
    <div class="rental frm_sty">
        <div class="rentalCont fr">
            <h3>应付金额:<span>￥<?php echo ($sum); ?></span></h3>
        </div>
    </div>
...
<script>
    ...
    $('#submit').click(function () {
        if($(':radio').is(":checked")){
            $.post("<?php echo u('Cart/submit_order');?>", $('#form1').serialize(), function (res) {
                if (res.status == 'ok') {
                    layer.alert(res.msg, {
                        yes: function () {
                            location.href = "<?php echo u('Member/order');?>";
                        }
                    });
                } else {
                    layer.alert(res.msg);
