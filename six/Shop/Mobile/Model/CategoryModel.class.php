        ...
        foreach ($data2 as $k => $v) {
            ...
            if (count($cates) > 0) {
                foreach ($cates as $k1 => $v1) {
                    ...
                    foreach ($data2[$k]['c'][$k1]['c'] as $key => $value) {
                        $image= M("Goods")->field("image")->where("cid=".$value["id"])->order("createtime desc")->limit("0,1")->find();
                        $data2[$k]['c'][$k1]['c'][$key]["image"]=reset(explode(",", $image["image"]));
                    }
                }
            }
