            ...
            if ($result && $result['status']<=0) {
                ...
                if(isset($hist)&&$hist>0){
                    ...
                    foreach($hist as $k3=>$v3){
                        $res=$hist1->where("gid=".$v3["gid"]." and uid=".$uid)->find();
                        if($res){
                ...
                if(isset($cart)&&$cart>0){
                    foreach($cart as $k1=>$v1){
                        $res=$cart1->where("gid=".$v1["gid"]." and uid=".$uid)->find();
                        if($res){
                            $buynum=$v1["buynum"]+$res["buynum"];
                            $cart1->where("gid=".$v1["gid"]." and uid=".$uid)->setField("buynum",$buynum);
                        }else {
                            $data1["gid"] = $v1["gid"];
                            $data1["addtime"] = $v1["addtime"];
                            $data1["buynum"] = $v1["buynum"];
                            $data1["uid"] = $uid;
                            $cart1->create($data1);
                            $cart1->add($data1);
                        }
                ...
                if(isset($collect)&&$collect>0){
                    foreach($collect as $k2=>$v2){
                        $res=$collect1->where("gid=".$v2["gid"]." and uid=".$uid)->find();
                        if($res){
                            if($res["status"]==0){
                                $collect1->where("gid=".$v2["gid"]." and uid=".$uid)->setField("status","1");
                            }
                        }
                        $info["gid"]=$v2["gid"];
                        $info["coltime"]=$v2["coltime"];
                        $info["status"]="1";
                        $info["uid"]=$uid;
