    ...
    public function memberInfo(){
        ...
        foreach($m2 as $val2){
            ...
            if($t){
                ...
                $page = new \Org\Yh\AjaxPage($total,3,'memberInfo');
//                $page -> rollPage =5;
...
//        $this->display();
    ...
    public function Code_send(){
        ...
        $users = M('Users');
        $user = $users->where(array('mobile' => $mobile))->field('id,mobile,nickname')->find();
        $phone = $user['mobile'];
        if($mobile==$user['mobile'] && $user['id']==session('uid')){
