$(document).ready(function() {
	...
	$("[mg]").hover(function(){
		...
		var  a1 = $(this).attr("mg");
		//$("[mg2="+a1+"]").show();
        $(this).children(".kj-if-show").show();
	},function(){
		$(this).removeClass("hover-show-bg");
		var  a1 = $(this).attr("mg");
		//$("[mg2="+a1+"]").hide();
        $(this).children(".kj-if-show").hide();
	});
...
//轮播图 END
	...
	/*$("[qie-xiao] li").hover(function() {
        var pic_ind_num=$(this).index();
        $('#pic_ind').val(pic_ind_num);
		$(this).addClass('current').siblings().removeClass('current');
	...
	$("[at]").click(function(event) {
		var ap = $(this).parents().siblings("[ap]").val();
		if(ap>0){
			ap++;
		}
        if(ap>100){
		...
		$(this).parents().siblings("[ap]").val(ap);
	});
