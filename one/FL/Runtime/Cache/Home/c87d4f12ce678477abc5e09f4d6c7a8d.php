    ...
    <div class="personal w1200">
        ...
        <div class="order-right f-r">
            ...
            <div class="order-md">
                ...
                <?php if(is_array($order)): $i = 0; $__LIST__ = array_slice($order,$fast,$end,true);if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><tr>
                   <td> <div style="height: 80px;line-height: 80px;"><span name="feel" onclick="orderd(this)"><a href="<?php echo u('User/ord',array('order_feel'=>$vo[order_feel]));?>"><?php echo ($vo["order_feel"]); ?></a></span></div></td>
                   <td><div style=""><?php echo date("Y-m-d H-i-s",$vo[order_date]);?></div></td>
                   <td><div style=""><?php echo ($vo["order_tot"]); ?></div></td>
                   <td><div style=""><?php echo ($vo["order_status"]); ?></div></td>
                   <td><div style="">
                       
                           <?php if($vo[order_status] == '已收货'): ?><a class="btn-style-01" href="<?php echo u('User/ord',array('order_feel'=>$vo[order_feel]));?>"> 未评论</a>
                               <?php elseif($vo[order_status] == '未付款'): ?>
                              <a class="btn-style-01" href="<?php echo u('Order/orderz',array('order_feel'=>$vo[order_feel]));?>">付款</a>
                               <?php elseif($vo[order_status] == '已评论'): ?>
                               <a class="btn-style-01" href="<?php echo u('User/ord',array('order_feel'=>$vo[order_feel]));?>"> 已评论</a>
                               <?php elseif($vo[order_status] == '已发货'): ?>
                                <a  class="btn-style-01" href="<?php echo u('User/qrsh',array('order_feel'=>$vo[order_feel]));?>">确认收货</a>
                               <?php elseif($vo[order_status] == '已付款'): ?>
                                <a class="btn-style-01" onclick="tx()">提醒发货</a><?php endif; ?>
...
<script>
    function xiangqing(e){
        $.get("<?php echo U('ord');?>",{'order_feel':e},function(res){
            layer.open({
                type: 2,
                area: ['800px', '500px'],
                fix: true, //不固定
                maxmin: true,
                content: "<?php echo U('ord');?>"
            });
        })
