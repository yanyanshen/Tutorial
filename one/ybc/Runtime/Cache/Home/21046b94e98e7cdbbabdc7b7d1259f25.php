    ...
    $(function(){
        mnext=function(id,status){
            if(status==3){
                ...
                location="<?php echo U('Order/newOrder');?>?id="+id;
            }else if(status==4){
                location="<?php echo U('Order/order');?>?oid="+id+"#Product_List";
            }
        }

        delOrder=function(id){
            layer.confirm("是否删除？",{icon:3},function(){
                $.post("<?php echo U('Order/delOrder');?>",{id:id},function(res){
                    if(res.status==1){
                        layer.msg(res.info,{time:1000},function(){
                            location.reload();
                        });
            ...
            <div class="user_Borders">
                ...
                <div class="Address_List">
                    <div class="title_name"><span class="name">订单列表</span>
                        <ul class="myul">
                            <a href="<?php echo U('myOrderList');?>?status=4"><li class="myli <?php echo ($status==4?'myliSel':''); ?>">待评价订单</li></a>
                            <a href="<?php echo U('myOrderList');?>?status=3"><li class="myli <?php echo ($status==3?'myliSel':''); ?>">待收货订单</li></a>
                            <a href="<?php echo U('myOrderList');?>?status=2"><li class="myli <?php echo ($status==2?'myliSel':''); ?>">待发货订单</li></a>
                            <a href="<?php echo U('myOrderList');?>?status=1"><li class="myli <?php echo ($status==1?'myliSel':''); ?>">代付款订单</li></a>
                            <a href="<?php echo U('myOrderList');?>"><li class="myli <?php echo ($status?'':'myliSel'); ?>">全部订单</li></a>
                        </ul>
                    </div>

                    <div class="list">
                        <?php if(empty($orderList)): ?><h1 style="color: red">还没有订单信息，赶紧购买下单去吧</h1>
                            <?php else: ?>
                        <table style="margin-bottom: 30px;">
                            ...
                            <?php if(is_array($orderList)): $i = 0; $__LIST__ = $orderList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$va): $mod = ($i % 2 );++$i;?><tr>
                                    <td><?php echo ($va["ordersyn"]); ?></td>
                                    <td><?php echo date("Y-m-d H:i:s",$va['addtime']);?></td>
                                    <td>￥<?php echo ($va["orderprice"]); ?></td>
                                    <td><?php echo ($va["status"]); ?></td>
                                    <td>
                                        <?php if(($va['orderstatus'] != 0)): ?><a href="<?php echo U('Order/order');?>?oid=<?php echo ($va["id"]); ?>">查看</a><?php endif; ?>
                                        <?php if(($va['orderstatus'] == 1) OR ($va['orderstatus'] == 0)): ?><a onclick="delOrder(<?php echo ($va["id"]); ?>)" style="cursor: pointer">删除</a><?php endif; ?>
                                        <?php if(($va['orderstatus'] == 1) OR ($va['orderstatus'] == 3) OR ($va['orderstatus'] == 4)): ?><a onclick="mnext(<?php echo ($va["id"]); ?>,<?php echo ($va["orderstatus"]); ?>);" style="cursor: pointer"><?php echo ($va["mnext"]); ?></a><?php endif; ?>
                                    </td>
