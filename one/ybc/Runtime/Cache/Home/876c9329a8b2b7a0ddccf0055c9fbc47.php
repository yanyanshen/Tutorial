    ...
    $(function(){
        ...
        cancel=function(id){
            layer.confirm("取消后不可再申请，确定取消吗？",{btn:["我要取消","我再考虑考虑"],title:"提示",icon:7},function(){
                $.post("<?php echo U('Order/cancelSer');?>",{id:id},function(res){
        ...
        chakan=function(id){
                ...
                layer.open({
                    type:2,
                    content:"<?php echo U('Order/detail');?>?id="+id,
                    title:"售后信息",
                    area:['1100px','800px']
                })
        ...
        <div class="right_style r_user_style">
            <div class="user_Borders">
                ...
                <div class="Address_List">
                    ...
                    <div class="list">
                        <?php if(empty($services)): ?><h1 style="color: red">你还没有任何申请售后记录！</h1>
                            <?php else: ?>
                            <table style="margin-bottom: 30px;">
                                ...
                                <?php if(is_array($services)): $i = 0; $__LIST__ = $services;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$va): $mod = ($i % 2 );++$i;?><tr>
                                        <td><a href="<?php echo U('Detail/detail');?>?id=<?php echo ($va["gid"]); ?>"><img src="/Uploads/goodsPic/100/100_<?php echo ($va["pic"]); ?>" alt=""/></a></td>
                                        <td><a href="<?php echo U('Detail/detail');?>?id=<?php echo ($va["gid"]); ?>"><?php echo ($va["goodsname"]); ?></a></td>
                                        <td><?php echo ($va["price"]); ?></td>
                                        <td><?php echo ($va["buynum"]); ?></td>
                                        <td><?php echo date("Y-m-d",$va['applytime']);?></td>
                                        <td><?php echo ($va['type']==1?"申请换货":"申请退货"); ?></td>
                                        <td>
                                            <?php if(($va["status"]) == "1"): ?>正在审核<?php endif; ?>
                                            <?php if(($va["status"]) == "2"): ?>未通过<?php endif; ?>
                                            <?php if(($va["status"]) == "3"): ?>已通过<?php endif; ?>
                                            <?php if(($va["status"]) == "4"): ?>已取消<?php endif; ?>
                                        </td>
                                        <td>
                                            <?php if(($va["status"]) == "1"): ?><a href="javascript:cancel(<?php echo ($va["id"]); ?>);">取消</a><?php endif; ?>
                                            <?php if(($va["status"]) == "1"): ?><a onclick="chakan(<?php echo ($va["id"]); ?>)" style="cursor: pointer">查看</a><?php endif; ?>
                                            <?php if(($va["status"]) == "2"): ?><a onclick="chakan(<?php echo ($va["id"]); ?>)" style="cursor: pointer">查看</a><?php endif; ?>
                                            <?php if(($va["status"]) == "3"): ?><a onclick="chakan(<?php echo ($va["id"]); ?>)" style="cursor: pointer">查看</a><?php endif; ?>
                                            <?php if(($va["status"]) == "4"): ?>无<?php endif; ?>
                                        </td>
