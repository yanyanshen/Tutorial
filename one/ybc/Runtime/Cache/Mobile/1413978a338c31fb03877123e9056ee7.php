...
<script type="text/javascript">
    $(function(){
        ...
        selAdd=function(id){
            $.get("<?php echo U('Order/selectAddr');?>",{ oid:<?php echo ($oid); ?>,aid:id},function(res){
                location.href="<?php echo U('Order/orderDetail');?>?id=<?php echo ($oid); ?>";
            })
        }
    })
</script>
...
<div style="display: inline-block; background:#eee;width: 100%;height: 2.5rem;line-height: 2.5rem;padding-left: 0.5rem;">
    <a href="<?php echo U('Order/orderDetail');?>?id=<?php echo ($oid); ?>" style="font-size: 1.3rem;text-decoration: none; color: #000;">&lt; 选择收货地址</a>
</div>
<?php if(is_array($addrList)): $k = 0; $__LIST__ = $addrList;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($k % 2 );++$k;?><div id="nav<?php echo ($k); ?>" class="nav">
        <ul onclick="selAdd(<?php echo ($v["id"]); ?>)">
        ...
        <div class="bot">
            ...
            <div onclick="selAdd(<?php echo ($v["id"]); ?>)" style="float: right;">选择</div>
        </div>
        <div></div>
    </div>
    <div id="fgx<?php echo ($k); ?>" style="width: 100%;height: 0.2rem;background: #ddd;"></div><?php endforeach; endif; else: echo "" ;endif; ?>
<div style="height: 2.1rem;"></div>
<div onclick="location.href='<?php echo U('Order/addAddr');?>?oid=<?php echo ($oid); ?>'" style="position: fixed;bottom: 0; width: 100%; background: #EE7712; color: #fff; font-size: 1.2rem;text-align: center; padding: 0.3rem;">添加新地址</div>
</body>
</html>