...
<script type="text/javascript">
    $(function(){
        ...
        $("#cExcel").click(function(){
            $.post("<?php echo U('IntegralOrder/export');?>?status=<?php echo ($status); ?>&keywords=<?php echo ($keywords); ?>&username=<?php echo ($username); ?>&date1=<?php echo ($date1); ?>&date2=<?php echo ($date2); ?>",'',function(res){
                if(res.status==1){
                    window.open("<?php echo U('IntegralOrder/export');?>?status=<?php echo ($status); ?>&keywords=<?php echo ($keywords); ?>&username=<?php echo ($username); ?>&date1=<?php echo ($date1); ?>&date2=<?php echo ($date2); ?>");
                }else{
...
<ul class="seachform">
<form action="<?php echo U('Admin/IntegralOrder/integralOrder');?>?<?php echo ($url1?$url1:''); ?>" method="post" id="form1">
    ...
    <li><label>&nbsp;</label><input type="submit" class="scbtn" value="查询"/></li>
       ...
       <?php if($status == 2): ?><li><label>&nbsp;</label><input id="plcz" type="button" class="scbtn" value="批量发货"/></li><?php endif; ?>
    <li><label>&nbsp;</label><input id="cExcel" type="button" class="scbtn" value="导出订单"/></li>
    ...
    <table class="tablelist">
        ...
        <?php if(is_array($list)): $k = 0; $__LIST__ = $list;if( count($__LIST__)==0 ) : echo "暂时没有数据" ;else: foreach($__LIST__ as $key=>$va): $mod = ($k % 2 );++$k;?><tr>
                ...
                <td><?php echo ($va["points"]); ?></td>
                <td><?php echo date("Y-m-d H:i:s",$va['addtime']);?></td>
                <td><?php echo ($va["status"]); ?></td>
                <td><a href="<?php echo U('Admin/IntegralOrder/orderDetail');?>?id=<?php echo ($va["id"]); ?>" class="tablelink">查看</a>
