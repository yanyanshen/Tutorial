<?php if (!defined('THINK_PATH')) exit();?><table border="0" class="jud_list" style="width:100%; margin-top:30px;" cellspacing="0" cellpadding="0">
    <?php if(is_array($commresponlist2)): $i = 0; $__LIST__ = $commresponlist2;if( count($__LIST__)==0 ) : echo "$empty2" ;else: foreach($__LIST__ as $key=>$comment2): $mod = ($i % 2 );++$i;?><tr valign="top">
            <td width="160"><img src="/UserImg/photo<?php echo ($comment2["imgpath"]); echo ($comment2["imgname"]); ?>" width="20" height="20" align="absmiddle" />&nbsp;<?php echo ($comment2["username"]); ?> <br /><font color="#999999">（匿名用户）</font></td>
            <td width="180">
                型号：<font color="#999999"><?php echo ($comment2["ml"]); ?>ml</font>
                <br /><font color="orange"><?php echo ($comment2["costatus"]); ?></font>
            </td>
            <td>
                <?php echo ($comment2["content"]); ?><br />
                <?php if($comment2["imageurl"] != ''&&$comment2["imagename"] != ''): ?><a href="javascript:;" class="showimg" ><img src="/CommentImg/<?php echo ($comment2["imageurl"]); echo ($comment2["imagename"]); ?>" alt="" style="width: 50px;height: 50px;"/></a>
                    <a href="<?php echo U('Home/Order/seeimg',array('cid'=>$comment2['id']));?>" target="_blank" class="seebigimg" style="margin-left: 10px;">查看原图</a><br/><?php endif; ?>
                <font color="#999999"><?php echo (date('Y-m-d',$comment2["coaddtime"])); ?></font>
            </td>
            <td>
                <?php if($comment2["respid"] == 2): ?><span style="color:orange;">卖家回复:</span><?php echo ($comment2["response"]); ?><br />
                    <font color="#999999"><?php echo (date('Y-m-d',$comment2["readdtime"])); ?></font><?php endif; ?>
            </td>
...
<table border="0" class="jud_list" style="width:100%; margin-top:30px;" cellspacing="0" cellpadding="0">
    <?php if(is_array($commresponlist['greate'])): $i = 0; $__LIST__ = $commresponlist['greate'];if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$val1): $mod = ($i % 2 );++$i;?><tr valign="top">
            ...
            <td>
                ...
                <?php if($comment2["imageurl"] != ''&&$comment2["imagename"] != ''): ?><a href="javascript:;" class="showimg" ><img src="/CommentImg/<?php echo ($val1["imageurl"]); echo ($val1["imagename"]); ?>" alt="" style="width: 50px;height: 50px;"/></a>
