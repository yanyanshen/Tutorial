...
<head>
    ...
    <style>
        ...
        .commlist td{text-align: center;line-height: 50px;margin: 0;padding: 0;}
        .commlist tr{text-align: center;line-height: 50px;margin: 0;padding: 0;}
        .commlist img{margin: 0;padding: 0;}
    </style>
    <script type="text/javascript">
        ...
        <div class="m_right">
            ...
            <div class="m_des">
                <table border="0" style="width:870px; line-height:22px;" cellspacing="0" cellpadding="0">
                    <?php if(is_array($userInfo)): $i = 0; $__LIST__ = $userInfo;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$user): $mod = ($i % 2 );++$i;?><tr valign="top">
                        <td width="115"><img src="/UserImg/photo<?php echo ($user["imgpath"]); echo ($user["imgname"]); ?>" width="90" height="90" /></td>
                        <td>
                            <div class="m_user"><?php echo ($user["username"]); ?></div>
                            <p>
                                等级：<?php echo ($user["vipname"]); ?> <br />
                                <font color="#ff4e00">您当前积分为：<?php echo ($user["score"]); ?>分</font><br />
                                上一次登录时间: <?php echo (date('Y-m-d H:i:s',$user["lastlogin"])); ?><br />
                            </p>
                            <div class="m_notice">
                        </div>
                        </td>
            ...
            <div class="mem_tit">
                <span class="fr" style="font-size:12px; color:#55555; font-family:'宋体'; margin-top:5px;">共发现&nbsp;<span style="color: red;font-weight: bold;font-size: 14px;"><?php echo ($count); ?></span>&nbsp;条记录</span>我的评论
            </div>
            <table border="0" class="order_tab" style="width:930px;" cellspacing="0" cellpadding="0">
                <tr>
                    <td align="center" width="420">商品图片</td>
                    <td align="center" width="180">评价</td>
                    <td align="center" width="180">类别</td>
                    <td align="center" width="180">追加</td>
                    <td align="center" width="180">回复</td>
                    <td align="center" width="180">操作</td>
                </tr>
                <form action="" method="get" id="comment">
                    <?php if(is_array($commentlist)): $i = 0; $__LIST__ = $commentlist;if( count($__LIST__)==0 ) : echo "$empty" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                            <td style="font-family:'宋体';">
                                <div class="sm_img">
                                    <a href="<?php echo U('Home/Order/goodsdetail',array('gid'=>$v['gid']));?>" style="width: 48px;height: 50px;display: inline-block;margin: 0;padding: 0;"><img src="/Uploads/<?php echo ($v["imageurl"]); ?>100_<?php echo ($v["imagename"]); ?>" alt="" width="48" height="50"/></a>
                                </div>
                                <span><?php echo ($v["ml"]); ?>ml</span>
                                <a href="<?php echo U('Home/Order/goodsdetail',array('gid'=>$v['gid']));?>"><?php echo mb_substr($v['goodsname'],0,8,'utf-8');?></a>
                            </td>
                            <td align="center"><?php echo ($v["content"]); ?></td>
                            <td style="text-align: center;"><span><?php echo ($v["costatus"]); ?></span><br/><span style="color: #ccc;"><?php echo (date('Y-m-d H:i:s',$v["coaddtime"])); ?></span></td>
                            <?php if($v["zuijiacount"] == 1): ?><td align="center"><?php echo ($v["zuijia"]); ?></td>
                                <?php else: ?>
                                <td align="center">用户还未追加哦</td><?php endif; ?>
                            <?php if($v["respid"] == 2): ?><td style="text-align: center;"><?php echo ($v["response"]); ?></td>
                                <?php else: ?>
                                <td style="text-align: center;">卖家还未做出回复</td><?php endif; ?>
                            <td style="text-align: center;"><a href="javascript:;" class="deletecomment" gid="<?php echo ($v["gid"]); ?>" oid="<?php echo ($v["oid"]); ?>">删除</a></td>
...
<script type="text/javascript">
    $(function(){
        $(".deletecomment").click(function(){
            ...
            a=$(this);
            $.get("<?php echo U('Member/deletecomment');?>",{gid:gid,oid:oid},function(res){
                if(res.status){
