    ...
    <table class="tablelist">
        ...
        <tbody>
            <?php if(is_array($list1)): $k = 0; $__LIST__ = $list1;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$date): $mod = ($k % 2 );++$k;?><tr>
                    ...
                    <td class="zhuangtai"><?php echo ($date['show']?'展示':'下架'); ?></td>
                    <td class="par">
                        <a href="javascript:;" pid="<?php echo ($date['pid']); ?>" id="<?php echo ($date['id']); ?>" class="tablelink click"><?php echo ($date['show']?'下架':'展示'); ?></a>
                        &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                        <a href="<?php echo U('Admin/Category/editorCate',array('id'=>$date['id']));?>" class="tablelink">编辑</a>
                    </td>
...
<script>
    ...
    $(function(){
        $('.click').click(function(){
            ...
            pid=$(this).attr('pid');
            cur=$(this);
            $.post("<?php echo U('Admin/Category/updateshow');?>",{id:id,pid:pid},function(response){
                if(response.status){
                    if(response.info==1){
                        cur.text('下架');
                        cur.parents('.par').prev('.zhuangtai').text('展示');
                        layer.msg('展示成功',{icon:1},function(){
                            location='<?php echo U("Category/index");?>';
                        });
                    }if(response.info==0){
                        cur.text('展示');
