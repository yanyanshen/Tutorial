...
<form action="<?php echo U('Admin/Golds/search');?>" method="get" id="form1">
    <ul class="seachform">
        ...
        <li><label>销售价格</label>
            <input name="saleprice1" type="text" class="scinput" value="<?php echo ($saleprice1); ?>" style="width: 60px;height: 25px;"/>
            <span style="display: inline-block;">~</span><input name="saleprice2" value="<?php echo ($saleprice2); ?>" type="text" class="scinput" style="width: 60px;height: 25px;"/></li>
        <li>
            <label>发布时间：</label>
            <input id="d11" type="text" value="<?php echo ($time1); ?>" onClick="WdatePicker()" style="width: 80px;height:25px;border: 1px solid #cccccc;" name="time1" />
            <span style="display: inline-block;">-</span>
            <input name="time2" class="Wdate" value="<?php echo ($time2); ?>" type="text" id="d15" onFocus="WdatePicker({isShowClear:false,readOnly:true})" style="width: 80px;height: 25px;border: 1px solid #cccccc;"/>
        </li>
    ...
    <table class="tablelist">
        ...
        <td style="font-size: 10px;" class="par">
            <a href="<?php echo U('Admin/Golds/editor',array('gid'=>$val['id']));?>"  class="tablelink">编辑&nbsp;&nbsp;&nbsp;&nbsp;</a>
            <a href="javascript:;" gid="<?php echo ($val['id']); ?>"  class="tablelink click"><?php echo ($val['show']==1?'下架':'展示'); ?></a></td>
        </tr><?php endforeach; endif; else: echo "$empty" ;endif; ?>
        </tbody>
    </table>
    
       <div class="pagin">
        <div class="message">共<i class="blue" name="count"><?php echo ($count); ?></i>条记录，当前显示第&nbsp;<i class="blue" name="current">&nbsp;<?php echo ($current); ?></i>页</div>
      <div class="paginList">
          <?php echo ($page); ?>
      </div>
    </div>
...
<script type="text/javascript">
    $(function(){
        $("#exportdata").click(function(){
            $.post("<?php echo U('Golds/export');?>?goodsname=<?php echo ($goodsname); ?>&bname=<?php echo ($bname); ?>&cname=<?php echo ($cname); ?>&time1=<?php echo ($time1); ?>&time2=<?php echo ($time2); ?>&price1=<?php echo ($price1); ?>&price2=<?php echo ($price2); ?>",'',function(res){
                if(res.status==1){
                    window.open("<?php echo U('Golds/export');?>?goodsname=<?php echo ($goodsname); ?>&bname=<?php echo ($bname); ?>&cname=<?php echo ($cname); ?>&time1=<?php echo ($time1); ?>&time2=<?php echo ($time2); ?>&price1=<?php echo ($price1); ?>&price2=<?php echo ($price2); ?>");
                }else{
    ...
    $(function(){
        $('.click').click(function(){
            gid=$(this).attr('gid');
            cur=$(this);
            $.post("<?php echo U('Admin/Golds/updateshow');?>",{gid:gid},function(response){
                if(response.status){
                    if(response.info==1){
                        cur.text('下架');
