...
<script src="/Public/Home/js/iCheck.js" type="text/javascript"></script>
   ...
   <div class="title_style"><em></em>购物车</div>
    ...
    <!--订单管理-->
      ...
      <div class="Order_Operation">
         ...
         <input name="" type="submit" value="清空购物车" id="send1" class="confirm_Order"/></div>
      ...
      <div class="Order_form_list">
          <form  method="get" action="<?php echo U('toPay');?>" id="form">
         <table>
         <thead>
          <tr>
          ...
          <td class="list_name_title4" style="width: 20%">小计(元)</td>
         ...
         <?php if(is_array($arr)): $i = 0; $__LIST__ = $arr;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$res): $mod = ($i % 2 );++$i;?><tr>
             ...
             <td colspan="6" class="Order_form_time">
                 <input  type="checkbox" value="<?php echo ($res["id"]); ?>" id="checked" checked="checked"/>
                 <span>添加时间:<?php echo ($time); ?></span>
             </td>
         </tr>
         <tr>
             <td>
             ...
             <td class="split_line" id="price" value="<?php echo ($res['price']); ?>"><?php echo ($res['price']); ?></td>
             <td>
                 <input type="text" value="<?php echo ($res["num"]); ?>" class="float-lt choose_input" name="num[<?php echo ($res["pid"]); ?>]" id="num" style="width: 20px"/>
             </td>
             <td class="split_line" id="prices"><?php echo ($res["prices"]); ?></td>
             <td class="operating">
                 <a href="">联系客服</a>
                 <a href="javascript:showCarDel(<?php echo ($res["id"]); ?>)">删除</a>
             </td>
         </tr><?php endforeach; endif; else: echo "" ;endif; ?>
         </table>
          <h1>总价:<span style="color: red;font-size: 40px">￥:<?php echo ($count); ?></span></h1>
          <a>
              <div style="width: 150px;height: 40px;margin: 50px;background-color: #cccbc8;font-size: 20px;line-height: 40px;text-align: center" id="send">生成订单</div>
          </a>
          </form>
    </div>
     </div>
   ...
   <script type="text/javascript">
       //全选
           $(document).ready(function(){
            ...
            function showCarDel($id){
                $.get("<?php echo U('delCar');?>",{'id':$id},function(res){
                    if(res.status=='ok'){
                        layer.alert(res.msg);
                    }else{
                        layer.alert(res.msg);
...
<div class="fri-link-bg clearfix">
    <div class="fri-link">
        <div class="logo left margin-r20"><img src="/Public/Home/images/fo-logo.jpg" width="152" height="81" /></div>
        <div class="left"><img src="/Public/Home/images/qd.jpg" width="90"  height="90" />
            <p>扫描下载APP</p>
        </div>
       <div class="">
    <dl>
